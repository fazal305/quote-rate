...
import type { CustomPage, PaymentStructure, QuoteDraft } from '@/types/quote'
...
function generateCustomPageId(): string {
...
interface CalculatorState {
  ...
  setComplexity: (complexityId: ComplexityLevel) => void
  setPageQuantity: (pageTypeId: string, quantity: number) => void
  addCustomPage: () => void
  updateCustomPage: (id: string, patch: Partial<Omit<CustomPage, 'id'>>) => void
  removeCustomPage: (id: string) => void
  toggleFeature: (featureId: string) => void
  setExperienceLevel: (id: string) => void
  toggleSpecialization: (id: string) => void
  setMarketPositioning: (id: MarketPositioning) => void
  setUrgency: (id: string) => void
  setContingencyPercent: (percent: number) => void
  setAdditionalRevisionRounds: (rounds: number) => void
  setMaintenancePlan: (id: string | null) => void
  toggleHostingItem: (id: string) => void
  setPaymentStructure: (value: PaymentStructure) => void
    ...
    (set) => ({
      ...
      setPageQuantity: (pageTypeId, quantity) =>
        set((s) => {
          ...
          let selectedPages: QuoteDraft['selectedPages']
          if (quantity <= 0) {
